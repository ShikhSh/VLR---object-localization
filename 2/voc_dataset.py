...
class VOCDataset(Dataset):
    ...
    def collate_fn(batch):
        ret = {}
        ret['image'] = torch.stack([d['image'] for d in batch], dim=0)
        ret['label'] = torch.stack([d['label'] for d in batch], dim=0)
        ret['wgt'] = [d['wgt'] for d in batch]
        ret['rois'] = [d['rois'] for d in batch]
        ret['gt_boxes'] = [d['gt_boxes'] for d in batch]
        ret['gt_classes'] = [d['gt_classes'] for d in batch]
