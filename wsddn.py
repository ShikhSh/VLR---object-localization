...
class WSDDN(nn.Module):
    ...
    def forward(self, image, rois=None, target_labels=None):
        ...
        # print(rois[0].shape)
        rois = rois.squeeze()
        roi_features = self.roi_pool(features, boxes = [rois.type('torch.FloatTensor').cuda()], output_size = (6,6), spatial_scale = 1.0*feat_dims/input_dims)
        ...
        cls_prob = torch.mul(score1, score2).clamp(0.0, 1.0)
